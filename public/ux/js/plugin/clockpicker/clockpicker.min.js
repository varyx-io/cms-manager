!function() {
    ...
    function d(d, h) {
        function j(a, b) {
            ...
            if (!b || !(t - v > n || n > t + v)) {
                ...
                k && m.append(D.canvas), D.setHand(j, l, !b, !0), i.off(o).on(o, function(a) {
                    ...
                    (b || q) && d === j && k === l && D.setHand(d, k), "hours" === D.currentView ? D.toggleView("minutes", x / 2) : h.autoclose && (D.minutesView.addClass("clockpicker-dial-out"), 
                    setTimeout(function() {
                        D.done();
                    }, x / 2)), m.prepend(L), clearTimeout(r), f.removeClass("clockpicker-moving"), 
                    i.off(o);
                });
            }
        }
        ...
        if (this.id = c("cp"), this.element = d, this.options = h, this.isAppended = !1, 
        this.isShown = !1, this.currentView = "hours", this.isInput = A, this.input = B, 
        this.addon = C, this.popover = l, this.plate = m, this.hoursView = q, this.minutesView = r, 
        this.amPmBlock = z, this.spanHours = l.find(".clockpicker-span-hours"), this.spanMinutes = l.find(".clockpicker-span-minutes"), 
        this.spanAmPm = l.find(".clockpicker-span-am-pm"), this.amOrPm = "PM", h.twelvehour) {
        ...
        h.autoclose || g('<button type="button" class="btn btn-sm btn-default btn-block clockpicker-button">' + h.donetext + "</button>").click(g.proxy(this.done, this)).appendTo(l), 
        "top" !== h.placement && "bottom" !== h.placement || "top" !== h.align && "bottom" !== h.align || (h.align = "left"), 
        "left" !== h.placement && "right" !== h.placement || "left" !== h.align && "right" !== h.align || (h.align = "top"), 
        l.addClass(h.placement), l.addClass("clockpicker-align-" + h.align), this.spanHours.click(g.proxy(this.toggleView, this, "hours")), 
        this.spanMinutes.click(g.proxy(this.toggleView, this, "minutes")), B.on("focus.clockpicker click.clockpicker", g.proxy(this.show, this)), 
        C.on("click.clockpicker", g.proxy(this.toggle, this));
        var F, G, H, I, J = g('<div class="clockpicker-tick"></div>');
        if (h.twelvehour) for (F = 1; 13 > F; F += 1) G = J.clone(), H = F / 6 * Math.PI, 
        I = t, G.css("font-size", "120%"), G.css({
        ...
        if (m.on(n, function(a) {
            ...
            var O = a("circle");
            O.setAttribute("class", "clockpicker-canvas-bearing"), O.setAttribute("cx", 0), 
            O.setAttribute("cy", 0), O.setAttribute("r", 2);
            var P = a("line");
            P.setAttribute("x1", 0), P.setAttribute("y1", 0);
            var Q = a("circle");
            Q.setAttribute("class", "clockpicker-canvas-bg"), Q.setAttribute("r", v);
            var R = a("circle");
            R.setAttribute("class", "clockpicker-canvas-fg"), R.setAttribute("r", 3.5), N.appendChild(P), 
            N.appendChild(Q), N.appendChild(R), N.appendChild(O), M.appendChild(N), L.append(M), 
            this.hand = P, this.bg = Q, this.fg = R, this.bearing = O, this.g = N, this.canvas = L;
        }
        e(this.options.init);
    }
    function e(a) {
    ...
    var f, g = window.jQuery, h = g(window), i = g(document), j = "http://www.w3.org/2000/svg", k = "SVGAngle" in window && function() {
        ...
        return b.innerHTML = "<svg/>", a = (b.firstChild && b.firstChild.namespaceURI) == j, 
        b.innerHTML = "", a;
    }(), l = function() {
    ...
    d.DEFAULTS = {
        ...
        if (!this.isShown) {
            ...
            this.hours = +d[0] || 0, this.minutes = +d[1] || 0, this.spanHours.html(b(this.hours)), 
            this.spanMinutes.html(b(this.minutes)), this.toggleView("hours"), this.locate(), 
            this.isShown = !0, i.on("click.clockpicker." + this.id + " focusin.clockpicker." + this.id, function(a) {
                var b = g(a.target);
        ...
        e(this.options.beforeHide), this.isShown = !1, i.off("click.clockpicker." + this.id + " focusin.clockpicker." + this.id), 
        i.off("keyup.clockpicker." + this.id), this.popover.hide(), e(this.options.afterHide);
    }, d.prototype.toggleView = function(a, b) {
        var c = !1;
        "minutes" === a && "visible" === g(this.hoursView).css("visibility") && (e(this.options.beforeHourSelect), 
        c = !0);
        var d = "hours" === a, f = d ? this.hoursView : this.minutesView, h = d ? this.minutesView : this.hoursView;
        this.currentView = a, this.spanHours.toggleClass("text-primary", d), this.spanMinutes.toggleClass("text-primary", !d), 
        h.addClass("clockpicker-dial-out"), f.css("visibility", "visible").removeClass("clockpicker-dial-out"), 
        this.resetClock(b), clearTimeout(this.toggleViewTimer), this.toggleViewTimer = setTimeout(function() {
        ...
        if (m.twelvehour && (o = t), 0 > h && (h = 2 * Math.PI + h), f = Math.round(h / j), 
        h = f * j, m.twelvehour ? i ? 0 === f && (f = 12) : (d && (f *= 5), 60 === f && (f = 0)) : i ? (12 === f && (f = 0), 
        f = n ? 0 === f ? 12 : f : 0 === f ? 0 : f + 12) : (d && (f *= 5), 60 === f && (f = 0)), 
        this[this.currentView] !== f && q && this.options.vibrate && (this.vibrateTimer || (navigator[q](10), 
        this.vibrateTimer = setTimeout(g.proxy(function() {
            this.vibrateTimer = null;
        }, this), 100))), this[this.currentView] = f, this[i ? "spanHours" : "spanMinutes"].html(b(f)), 
        !k) return void this[i ? "hoursView" : "minutesView"].find(".clockpicker-tick").each(function() {
            var a = g(this);
            a.toggleClass("active", f === +a.html());
        });
        e || !i && f % 5 ? (this.g.insertBefore(this.hand, this.bearing), this.g.insertBefore(this.bg, this.fg), 
        this.bg.setAttribute("class", "clockpicker-canvas-bg clockpicker-canvas-bg-trans")) : (this.g.insertBefore(this.hand, this.bg), 
        this.g.insertBefore(this.fg, this.bg), this.bg.setAttribute("class", "clockpicker-canvas-bg"));
        var p = Math.sin(h) * o, r = -Math.cos(h) * o;
        this.hand.setAttribute("x2", p), this.hand.setAttribute("y2", r), this.bg.setAttribute("cx", p), 
        this.bg.setAttribute("cy", r), this.fg.setAttribute("cx", p), this.fg.setAttribute("cy", r);
    }, d.prototype.done = function() {
        e(this.options.beforeDone), this.hide();
        var a = this.input.prop("value"), c = b(this.hours) + ":" + b(this.minutes);
        this.options.twelvehour && (c += this.amOrPm), this.input.prop("value", c), c !== a && (this.input.triggerHandler("change"), 
        this.isInput || this.element.trigger("change")), this.options.autoclose && this.input.trigger("blur"), 
        e(this.options.afterDone);
    }, d.prototype.remove = function() {
        this.element.removeData("clockpicker"), this.input.off("focus.clockpicker click.clockpicker"), 
        this.addon.off("click.clockpicker"), this.isShown && this.hide(), this.isAppended && (h.off("resize.clockpicker" + this.id), 
        this.popover.remove());
