(function() {
    ...
    b = window.Morris = {}, a = jQuery, b.EventEmitter = function() {
        ...
        function d(b) {
            var c = this;
            if ("string" == typeof b.element ? this.el = a(document.getElementById(b.element)) : this.el = a(b.element), 
            ...
            this.elementWidth = null, this.elementHeight = null, this.dirty = !1, this.init && this.init(), 
            this.setData(this.options.data), this.el.bind("mousemove", function(a) {
            ...
            }), this.el.bind("mouseout", function(a) {
                ...
                c.fire("hover", d.pageX - b.left, d.pageY - b.top), d;
        ...
        return g(d, c), d.prototype.gridDefaults = {
            ...
            return null == c && (c = !0), null == a || 0 === a.length ? (this.data = [], this.raphael.clear(), 
            void (null != this.hover && this.hover.hide())) : (m = this.cumulative ? 0 : null, 
            n = this.cumulative ? 0 : null, this.options.goals.length > 0 && (h = Math.min.apply(null, this.options.goals), 
            g = Math.max.apply(null, this.options.goals), n = null != n ? Math.min(n, h) : h, 
            m = null != m ? Math.max(m, g) : g), this.data = function() {
                var c, d, g;
                for (g = [], f = c = 0, d = a.length; d > c; f = ++c) j = a[f], i = {}, i.label = j[this.options.xkey], 
                this.options.parseTime ? (i.x = b.parseDate(i.label), this.options.dateFormat ? i.label = this.options.dateFormat(i.x) : "number" == typeof i.label && (i.label = new Date(i.label).toString())) : i.x = f, 
                k = 0, i.y = function() {
                    var a, b, c, d;
                    for (c = this.options.ykeys, d = [], e = a = 0, b = c.length; b > a; e = ++a) l = c[e], 
                    o = j[l], "string" == typeof o && (o = parseFloat(o)), null != o && "number" != typeof o && (o = null), 
                    null != o && (this.cumulative ? k += o : null != m ? (m = Math.max(o, m), n = Math.min(o, n)) : m = n = o), 
                    this.cumulative && null != k && (m = Math.max(k, m), n = Math.min(k, n)), d.push(o);
                    return d;
                }.call(this), g.push(i);
                return g;
            ...
            this.xmax += 1), this.ymin = this.yboundary("min", n), this.ymax = this.yboundary("max", m), 
            this.ymin === this.ymax && (n && (this.ymin -= 1), this.ymax += 1), this.yInterval = (this.ymax - this.ymin) / (this.options.numLines - 1), 
            ...
            var a, b, c;
            return c = this.el.width(), a = this.el.height(), (this.elementWidth !== c || this.elementHeight !== a || this.dirty) && (this.elementWidth = c, 
            this.elementHeight = a, this.dirty = !1, this.left = this.options.padding, this.right = this.elementWidth - this.options.padding, 
            this.top = this.options.padding, this.bottom = this.elementHeight - this.options.padding, 
            this.options.axes && (b = Math.max(this.measureText(this.yAxisFormat(this.ymin), this.options.gridTextSize).width, this.measureText(this.yAxisFormat(this.ymax), this.options.gridTextSize).width), 
            this.left += b, this.bottom -= 1.5 * this.options.gridTextSize), this.width = this.right - this.left, 
            this.height = this.bottom - this.top, this.dx = this.width / (this.xmax - this.xmin), 
            ...
            var c, d;
            return null == b && (b = 12), d = this.raphael.text(100, 100, a).attr("font-size", b), 
            ...
            if (this.options.grid !== !1 || this.options.axes !== !1) {
                for (a = this.ymin, b = this.ymax, h = [], c = f = a, g = this.yInterval; b >= a ? b >= f : f >= b; c = f += g) d = parseFloat(c.toFixed(this.precision)), 
                e = this.transY(d), this.options.axes && this.drawYAxisLabel(this.left - this.options.padding / 2, e, this.yAxisFormat(d)), 
                this.options.grid ? h.push(this.drawGridLine("M" + this.left + "," + e + "H" + (this.left + this.width))) : h.push(void 0);
                return h;
            }
        ...
        function c(a) {
            return this.hilight = h(this.hilight, this), this.onHoverOut = h(this.onHoverOut, this), 
            this.onHoverMove = h(this.onHoverMove, this), this instanceof b.Line ? void c.__super__.constructor.call(this, a) : new b.Line(a);
        }
        return g(c, a), c.prototype.init = function() {
            ...
            for (g = this.data.slice(1), c = e = 0, f = g.length; f > e && (d = g[c], !(a < (d._x + this.data[c]._x) / 2)); c = ++e) ;
            return c;
            ...
            var b, c, d, e, f, g, h;
            if (d = this.data[a], "function" == typeof this.options.hoverCallback) b = this.options.hoverCallback(a, this.options); else for (b = "<div class='morris-hover-row-label'>" + d.label + "</div>", 
            ...
            return this.paths = function() {
                var g, h, j, k;
                for (k = [], d = g = 0, h = this.options.ykeys.length; h >= 0 ? h > g : g > h; d = h >= 0 ? ++g : --g) f = this.options.smooth === !0 || (j = this.options.ykeys[d], 
                i.call(this.options.smooth, j) >= 0), c = function() {
                    var a, b, c, f;
            ...
            for (b = e = g = this.options.ykeys.length - 1; 0 >= g ? 0 >= e : e >= 0; b = 0 >= g ? ++e : --e) c = this.paths[b], 
            null !== c && this.drawLinePath(c, this.colorFor(d, b, "line"));
            for (this.seriesPoints = function() {
                ...
                var c, e, f, g;
                for (f = this.data, g = [], c = 0, e = f.length; e > c; c++) d = f[c], a = null != d._y[b] ? this.drawLinePoint(d._x, d._y[b], this.options.pointSize, this.colorFor(d, b, "point"), b) : null, 
                g.push(this.seriesPoints[b].push(a));
                return g;
            }.call(this));
            return i;
        ...
        "5sec": d(5),
        second: d(1)
    }, b.AUTO_LABEL_ORDER = [ "decade", "year", "month", "day", "hour", "30min", "15min", "10min", "5min", "minute", "30sec", "15sec", "10sec", "5sec", "second" ], 
    b.Area = function(a) {
        function c(a) {
        ...
        return g(c, a), c.prototype.calcPoints = function() {
            ...
            var a, b, d, e;
            for (a = d = e = this.options.ykeys.length - 1; 0 >= e ? 0 >= d : d >= 0; a = 0 >= e ? ++d : --d) b = this.paths[a], 
        ...
        function d(c) {
            return this.onHoverOut = h(this.onHoverOut, this), this.onHoverMove = h(this.onHoverMove, this), 
            this instanceof b.Bar ? void d.__super__.constructor.call(this, a.extend({}, c, {
                parseTime: !1
            })) : new b.Bar(c);
        }
        return g(d, c), d.prototype.init = function() {
            ...
            var a, b, c, d, e, f, g, h, i;
            for (f = this.bottom + 1.25 * this.options.gridTextSize, d = null, i = [], a = g = 0, 
            h = this.data.length; h >= 0 ? h > g : g > h; a = h >= 0 ? ++g : --g) e = this.data[this.data.length - 1 - a], 
            b = this.drawXAxisLabel(e._x, f, e.label), c = b.getBBox(), (null == d || d >= c.x + c.width) && c.x >= 0 && c.x + c.width < this.el.width() ? i.push(d = c.x - this.options.xLabelMargin) : i.push(b.remove());
            return i;
        }, d.prototype.drawSeries = function() {
            var a, b, c, d, e, f, g, h, i, j, k, l, m, n;
            return c = this.width / this.options.data.length, h = null != this.options.stacked ? 1 : this.options.ykeys.length, 
            a = (c * this.options.barSizeRatio - this.options.barGap * (h - 1)) / h, g = c * (1 - this.options.barSizeRatio) / 2, 
            n = this.ymin <= 0 && this.ymax >= 0 ? this.transY(0) : null, this.bars = function() {
                var h, o, p, q;
                for (p = this.data, q = [], d = h = 0, o = p.length; o > h; d = ++h) i = p[d], e = 0, 
                q.push(function() {
                    var h, o, p, q;
                    for (p = i._y, q = [], j = h = 0, o = p.length; o > h; j = ++h) m = p[j], null !== m ? (n ? (l = Math.min(m, n), 
                    b = Math.max(m, n)) : (l = m, b = this.bottom), f = this.left + d * c + g, this.options.stacked || (f += j * (a + this.options.barGap)), 
                    k = b - l, this.options.stacked && (l -= e), this.drawBar(f, l, a, k, this.colorFor(i, j, "bar")), 
            ...
            var b, c, d, e, f, g, h, i;
            if ("function" == typeof this.options.hoverCallback) b = this.options.hoverCallback(a, this.options); else for (d = this.data[a], 
            b = "<div class='morris-hover-row-label'>" + d.label + "</div>", i = d.y, c = g = 0, 
            h = i.length; h > g; c = ++g) f = i[c], b += "<div class='morris-hover-point' style='color: " + this.colorFor(d, c, "label") + "'>\n  " + this.options.labels[c] + ":\n  " + this.yLabelFormat(f) + "\n</div>";
        ...
        function c(c) {
            ...
            if ("string" == typeof c.element ? this.el = a(document.getElementById(c.element)) : this.el = a(c.element), 
            this.options = a.extend({}, this.defaults, c), null === this.el || 0 === this.el.length) throw new Error("Graph placeholder not found.");
        ...
        return c.prototype.defaults = {
            ...
            for (this.el.empty(), this.raphael = new Raphael(this.el[0]), c = this.el.width() / 2, 
            d = this.el.height() / 2, m = (Math.min(c, d) - 10) / 3, l = 0, u = this.data, o = 0, 
            r = u.length; r > o; o++) n = u[o], l += n.value;
            for (i = 5 / (2 * m), a = 1.9999 * Math.PI - i * this.data.length, g = 0, f = 0, 
            this.segments = [], v = this.data, p = 0, s = v.length; s > p; p++) e = v[p], j = g + i + a * (e.value / l), 
            k = new b.DonutSegment(c, d, 2 * m, m, g, j, this.options.colors[f % this.options.colors.length], this.options.backgroundColor, e, this.raphael), 
            k.render(), this.segments.push(k), k.on("hover", this.select), g = j, f += 1;
            for (this.text1 = this.drawEmptyDonutLabel(c, d - 10, this.options.labelColor, 15, 800), 
            this.text2 = this.drawEmptyDonutLabel(c, d + 10, this.options.labelColor, 14), h = Math.max.apply(null, function() {
                var a, b, c, d;
        ...
        function b(a, b, c, d, e, f, g, i, j, k) {
            this.cx = a, this.cy = b, this.inner = c, this.outer = d, this.color = g, this.backgroundColor = i, 
            this.data = j, this.raphael = k, this.deselect = h(this.deselect, this), this.select = h(this.select, this), 
            this.sin_p0 = Math.sin(e), this.cos_p0 = Math.cos(e), this.sin_p1 = Math.sin(f), 
            this.cos_p1 = Math.cos(f), this.is_long = f - e > Math.PI ? 1 : 0, this.path = this.calcSegment(this.inner + 3, this.inner + this.outer - 5), 
