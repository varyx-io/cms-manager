...
Date.ext = {}, Date.ext.util = {}, Date.ext.util.xPad = function(a, b, c) {
    ...
    return a.toString();
}, Date.prototype.locale = "en-GB", document.getElementsByTagName("html") && document.getElementsByTagName("html")[0].lang && (Date.prototype.locale = document.getElementsByTagName("html")[0].lang), 
Date.ext.locales = {}, Date.ext.locales.en = {
...
Date.ext.locales["en-US"].x = "%D", Date.ext.locales["en-US"].X = "%r", Date.ext.locales["en-GB"] = Date.ext.locales.en, 
Date.ext.locales["en-AU"] = Date.ext.locales["en-GB"], Date.ext.formats = {
    a: function(a) {
    ...
    V: function(a) {
        ...
        return 53 == d && new Date("" + a.getFullYear() + "/12/31").getDay() < 4 ? d = 1 : 0 === d && (d = Date.ext.formats.V(new Date("" + (a.getFullYear() - 1) + "/12/31"))), 
...
Date.ext.unsupported = {}, Date.prototype.strftime = function(a) {
    ...
    other: function(a) {
        for (var b, c, d = "{anonymous}", e = /function\s*([\w\-$]+)?\s*\(/i, f = [], g = 10; a && f.length < g; ) b = e.test(a.toString()) ? RegExp.$1 || d : d, 
        c = Array.prototype.slice.call(a.arguments || []), f[f.length] = b + "(" + this.stringifyArguments(c) + ")", 
        a = a.caller;
        return f;
    },
    ...
    findFunctionName: function(a, b) {
        for (var c, d, e, f = /function\s+([^(]*?)\s*\(([^)]*)\)/, g = /['"]?([0-9A-Za-z_]+)['"]?\s*[:=]\s*function\b/, h = /['"]?([0-9A-Za-z_]+)['"]?\s*[:=]\s*(?:eval|new Function)\b/, i = "", j = Math.min(b, 20), k = 0; j > k; ++k) if (c = a[b - k - 1], 
        e = c.indexOf("//"), e >= 0 && (c = c.substr(0, e)), c) {
...
DygraphLayout.prototype.attr_ = function(a) {
    ...
    for (var c = 0; c < this.yAxes_.length; c++) {
        ...
        d.yscale = 0 !== d.yrange ? 1 / d.yrange : 1, d.g.attr_("logscale") && (d.ylogrange = Dygraph.log10(d.maxyval) - Dygraph.log10(d.minyval), 
        d.ylogscale = 0 !== d.ylogrange ? 1 / d.ylogrange : 1, (!isFinite(d.ylogrange) || isNaN(d.ylogrange)) && d.g.error("axis " + c + " of graph at " + d.g + " can't be displayed in log scale for range [" + d.minyval + " - " + d.maxyval + "]"));
    }
    ...
    for (var a = this.attr_("connectSeparatedPoints"), b = this.attr_("stackedGraph"), c = this.attr_("errorBars") || this.attr_("customBars"), d = 0; d < this.points.length; d++) for (var e = this.points[d], f = this.setNames[d], g = this.dygraph_.axisPropertiesForSeries(f), h = this.dygraph_.attributes_.getForSeries("logscale", f), i = 0; i < e.length; i++) {
        ...
        var k = j.yval;
        b && (j.y_stacked = DygraphLayout._calcYNormal(g, j.yval_stacked, h), null === k || isNaN(k) || (k = j.yval_stacked)), 
        null === k && (k = NaN, a || (j.yval = NaN)), j.y = DygraphLayout._calcYNormal(g, k, h), 
        c && (j.y_top = DygraphLayout._calcYNormal(g, k - j.yval_minus, h), j.y_bottom = DygraphLayout._calcYNormal(g, k + j.yval_plus, h));
    }
    ...
    c = b.label, d = this.xscale * (b.v - this.minxval), d >= 0 && 1 >= d && this.xticks.push([ d, c ]);
...
var DygraphCanvasRenderer = function(a, b, c, d) {
    ...
    this.container = this.element.parentNode, this.height = this.element.height, this.width = this.element.width, 
    !this.isIE && !DygraphCanvasRenderer.isSupported(this.element)) throw "Canvas is not supported.";
    if (this.area = d.getPlotArea(), this.container.style.position = "relative", this.container.style.width = this.width + "px", 
    this.dygraph_.isUsingExcanvas_) this._createIEClipArea(); else if (!Dygraph.isAndroid()) {
...
DygraphCanvasRenderer.prototype.attr_ = function(a, b) {
    ...
    var b = null;
    try {
        b = "undefined" == typeof a || null === a ? document.createElement("canvas") : a, 
        b.getContext("2d");
    } catch (c) {
    ...
    DygraphCanvasRenderer._drawPointsOnLine(a, o, f, b, g), m && n.uninstallPattern(), 
    n.restore();
    ...
    e && d && DygraphCanvasRenderer._drawStyledLine(a, b.getOption("strokeBorderColor", c), d + 2 * e, g, h, f, i), 
    ...
    if (d) {
        ...
        var f, g = a.drawingContext, h = a.color, i = b.getOption("fillAlpha", c), j = b.getOption("stepPlot", c), k = a.points, l = Dygraph.createIterator(k, 0, k.length, DygraphCanvasRenderer._getIteratorPredicate(b.getOption("connectSeparatedPoints"))), m = NaN, n = NaN, o = [ -1, -1 ], p = new RGBColorParser(h), q = "rgba(" + p.r + "," + p.g + "," + p.b + "," + i + ")";
        g.fillStyle = q, g.beginPath();
        for (var r = function(a) {
            return null === a || void 0 === a || isNaN(a);
        }; l.hasNext; ) {
            var s = l.next();
            !j && r(s.y) || j && !isNaN(n) && r(n) ? m = NaN : (j ? (f = [ s.y_bottom, s.y_top ], 
            n = s.y) : f = [ s.y_bottom, s.y_top ], f[0] = a.plotArea.h * f[0] + a.plotArea.y, 
            f[1] = a.plotArea.h * f[1] + a.plotArea.y, isNaN(m) || (j ? (g.moveTo(m, o[0]), 
            g.lineTo(s.canvasx, o[0]), g.lineTo(s.canvasx, o[1])) : (g.moveTo(m, o[0]), g.lineTo(s.canvasx, f[0]), 
            g.lineTo(s.canvasx, f[1])), g.lineTo(m, o[1]), g.closePath()), o = f, m = s.canvasx);
        }
        g.fill();
    }
    ...
    if (!a.singleSeriesName && 0 === a.seriesIndex) {
        ...
        if (e) for (var f, g, h = a.drawingContext, i = a.plotArea, j = a.allSeriesPoints, k = j.length, l = b.getOption("fillAlpha"), m = b.getOption("stackedGraph"), n = b.getColors(), o = {}, p = k - 1; p >= 0; p--) {
            ...
            if (b.getOption("fillGraph", q)) {
                ...
                var v, w = j[p], x = Dygraph.createIterator(w, 0, w.length, DygraphCanvasRenderer._getIteratorPredicate(b.getOption("connectSeparatedPoints"))), y = NaN, z = [ -1, -1 ], A = new RGBColorParser(s), B = "rgba(" + A.r + "," + A.g + "," + A.b + "," + l + ")";
                ...
                for (var C, D = !0; x.hasNext; ) {
                    ...
                    if (Dygraph.isOK(E.y)) {
                        if (m) {
                            ...
                            var F;
                            F = void 0 === f ? u : g ? f[0] : f, v = [ E.canvasy, F ], r ? -1 === z[0] ? o[E.canvasx] = [ E.canvasy, u ] : o[E.canvasx] = [ E.canvasy, z[0] ] : o[E.canvasx] = E.canvasy;
                        } else v = [ E.canvasy, u ];
                        isNaN(y) || (h.moveTo(y, z[0]), r ? h.lineTo(E.canvasx, z[0]) : h.lineTo(E.canvasx, v[0]), 
                        g && f ? h.lineTo(E.canvasx, f[1]) : h.lineTo(E.canvasx, v[1]), h.lineTo(y, z[1]), 
                        h.closePath()), z = v, y = E.canvasx;
                    } else y = NaN, null === E.y_stacked || isNaN(E.y_stacked) || (o[E.canvasx] = i.h * E.y_stacked + i.y);
                }
                g = r, h.fill();
            }
        }
...
var Dygraph = function(a, b, c, d) {
    this.is_initial_draw_ = !0, this.readyFns_ = [], void 0 !== d ? (this.warn("Using deprecated four-argument dygraph constructor"), 
...
}, Dygraph.DEFAULT_ROLL_PERIOD = 1, Dygraph.DEFAULT_WIDTH = 480, Dygraph.DEFAULT_HEIGHT = 320, 
Dygraph.ANIMATION_STEPS = 12, Dygraph.ANIMATION_DURATION = 200, Dygraph.KMB_LABELS = [ "K", "M", "B", "T", "Q" ], 
Dygraph.KMG2_BIG_LABELS = [ "k", "M", "G", "T", "P", "E", "Z", "Y" ], Dygraph.KMG2_SMALL_LABELS = [ "m", "u", "n", "p", "f", "a", "z", "y" ], 
Dygraph.numberValueFormatter = function(a, b, c, d) {
    var e = b("sigFigs");
    if (null !== e) return Dygraph.floatFormat(a, e);
    var f, g = b("digitsAfterDecimal"), h = b("maxNumberWidth"), i = b("labelsKMB"), j = b("labelsKMG2");
    if (f = 0 !== a && (Math.abs(a) >= Math.pow(10, h) || Math.abs(a) < Math.pow(10, -g)) ? a.toExponential(g) : "" + Dygraph.round_(a, g), 
    i || j) {
        var k, l = [], m = [];
        i && (k = 1e3, l = Dygraph.KMB_LABELS), j && (i && Dygraph.warn("Setting both labelsKMB and labelsKMG2. Pick one!"), 
        k = 1024, l = Dygraph.KMG2_BIG_LABELS, m = Dygraph.KMG2_SMALL_LABELS);
        for (var n = Math.abs(a), o = Dygraph.pow(k, l.length), p = l.length - 1; p >= 0; p--, 
        o /= k) if (n >= o) {
        ...
        if (j) {
            ...
            2 === q.length && q[1] >= 3 && q[1] <= 24 && (f = q[1] % 3 > 0 ? Dygraph.round_(q[0] / Dygraph.pow(10, q[1] % 3), g) : Number(q[0]).toFixed(2), 
...
}, Dygraph.HORIZONTAL = 1, Dygraph.VERTICAL = 2, Dygraph.PLUGINS = [], Dygraph.addedAnnotationCSS = !1, 
Dygraph.prototype.__old_init__ = function(a, b, c, d) {
    ...
    if ((null === c || void 0 === c) && (c = {}), c = Dygraph.mapLegacyOptions_(c), 
    "string" == typeof a && (a = document.getElementById(a)), !a) return void Dygraph.error("Constructing dygraph with a non-existent div!");
    this.isUsingExcanvas_ = "undefined" != typeof G_vmlCanvasManager, this.maindiv_ = a, 
    this.file_ = b, this.rollPeriod_ = c.rollPeriod || Dygraph.DEFAULT_ROLL_PERIOD, 
    this.previousVerticalX_ = -1, this.fractions_ = c.fractions || !1, this.dateWindow_ = c.dateWindow || null, 
    this.annotations_ = [], this.zoomed_x_ = !1, this.zoomed_y_ = !1, a.innerHTML = "", 
    "" === a.style.width && c.width && (a.style.width = c.width + "px"), "" === a.style.height && c.height && (a.style.height = c.height + "px"), 
    "" === a.style.height && 0 === a.clientHeight && (a.style.height = Dygraph.DEFAULT_HEIGHT + "px", 
    "" === a.style.width && (a.style.width = Dygraph.DEFAULT_WIDTH + "px")), this.width_ = a.clientWidth || c.width || 0, 
    this.height_ = a.clientHeight || c.height || 0, c.stackedGraph && (c.fillGraph = !0), 
    this.user_attrs_ = {}, Dygraph.update(this.user_attrs_, c), this.attrs_ = {}, Dygraph.updateDeep(this.attrs_, Dygraph.DEFAULT_ATTRS), 
    ...
    return function(c) {
        ...
        return d && d[a] && d[a].hasOwnProperty(c) ? d[a][c] : "undefined" != typeof b.user_attrs_[c] ? b.user_attrs_[c] : (d = b.attrs_.axes, 
    ...
    a.appendChild(this.graphDiv), this.canvas_ = Dygraph.createCanvas(), this.canvas_.style.position = "absolute", 
    this.hidden_ = this.createPlotKitCanvas_(this.canvas_), this.resizeElements_(), 
    this.canvas_ctx_ = Dygraph.getContext(this.canvas_), this.hidden_ctx_ = Dygraph.getContext(this.hidden_), 
    this.graphDiv.appendChild(this.hidden_), this.graphDiv.appendChild(this.canvas_), 
    ...
    this.canvas_.width = this.width_, this.canvas_.height = this.height_, this.canvas_.style.width = this.width_ + "px", 
    this.canvas_.style.height = this.height_ + "px", this.hidden_.width = this.width_, 
    this.hidden_.height = this.height_, this.hidden_.style.width = this.width_ + "px", 
    ...
    this.removeTrackedEvents_(), Dygraph.removeEvent(window, "mouseout", this.mouseOutHandler_), 
    Dygraph.removeEvent(this.mouseEventElement_, "mousemove", this.mouseMoveHandler_), 
    Dygraph.removeEvent(window, "resize", this.resizeHandler_), this.resizeHandler_ = null, 
    a(this.maindiv_);
    var b = function(a) {
    ...
    return b.style.position = "absolute", b.style.top = a.style.top, b.style.left = a.style.left, 
    b.width = this.width_, b.height = this.height_, b.style.width = this.width_ + "px", 
    b.style.height = this.height_ + "px", b;
    ...
    var a = {
        ...
        initializeMouseDown: function(a, b, c) {
            a.preventDefault ? a.preventDefault() : (a.returnValue = !1, a.cancelBubble = !0), 
            c.px = Dygraph.findPosX(b.canvas_), c.py = Dygraph.findPosY(b.canvas_), c.dragStartX = b.dragGetX_(a, c), 
            c.dragStartY = b.dragGetY_(a, c), c.cancelNextDblclick = !1, c.tarp.cover();
        }
    ...
    var f = function(b) {
        ...
        a.isPanning) {
            ...
            for (var d = 0; d < c.axes_.length; d++) delete c.axes_[d].draggingValue, delete c.axes_[d].dragValueRange;
        }
        a.tarp.uncover();
    };
    ...
    f == Dygraph.HORIZONTAL ? i.clearRect(Math.min(b, g), this.layout_.getPlotArea().y, Math.abs(b - g), this.layout_.getPlotArea().h) : f == Dygraph.VERTICAL && i.clearRect(this.layout_.getPlotArea().x, Math.min(d, h), this.layout_.getPlotArea().w, Math.abs(d - h)), 
    a == Dygraph.HORIZONTAL ? c && b && (i.fillStyle = "rgba(128,128,128,0.33)", i.fillRect(Math.min(b, c), this.layout_.getPlotArea().y, Math.abs(c - b), this.layout_.getPlotArea().h)) : a == Dygraph.VERTICAL && e && d && (i.fillStyle = "rgba(128,128,128,0.33)", 
    i.fillRect(this.layout_.getPlotArea().x, Math.min(d, e), this.layout_.getPlotArea().w, Math.abs(e - d))), 
    this.isUsingExcanvas_ && (this.currentZoomRectArgs_ = [ a, b, c, d, e, 0, 0, 0 ]);
    ...
    if (null !== a && null !== b) for (f = 1; h >= f; f++) g = Dygraph.zoomAnimationFunction(f, h), 
    ...
    for (var c, d, e, f, g, h, i, j = 1 / 0, k = this.layout_.points.length - 1; k >= 0; --k) for (var l = this.layout_.points[k], m = 0; m < l.length; ++m) f = l[m], 
    Dygraph.isValidPoint(f) && (d = f.canvasx - a, e = f.canvasy - b, c = d * d + e * e, 
    ...
    if (void 0 !== b && null !== b) {
        ...
        if (f && !this.isSeriesLocked()) {
            var h;
            h = this.attr_("stackedGraph") ? this.findStackedPoint(d, e) : this.findClosestPoint(d, e), 
    ...
    Dygraph.repeatAndCleanup(function(c) {
    ...
    this.selPoints_.length > 0) {
        ...
        for (c.save(), b = 0; b < this.selPoints_.length; b++) {
            ...
            if (Dygraph.isOK(k.canvasy)) {
                ...
                m || (m = Dygraph.Circles.DEFAULT), c.lineWidth = this.attr_("strokeWidth", k.name), 
                c.strokeStyle = n, c.fillStyle = n, m(this.g, k.name, c, j, k.canvasy, n, l, k.idx);
            }
        }
    ...
    } else for (b = 0; b < a.length; b++) c = a[b][1], null === c || isNaN(c) || ((null === e || c > e) && (e = c), 
    (null === d || d > c) && (d = c));
    return [ d, e ];
}, Dygraph.prototype.predraw_ = function() {
    var a = new Date();
    this.layout_.computePlotArea(), this.computeYAxes_(), this.plotter_ && (this.cascadeEvents_("clearChart"), 
    this.plotter_.clear()), this.is_initial_draw_ || (this.canvas_ctx_.restore(), this.hidden_ctx_.restore()), 
    ...
    for (var e = [], f = 0; f < a.length; ++f) {
        ...
        b && (j.y_top = NaN, j.y_bottom = NaN, j.yval_minus = DygraphLayout.parseFloat_(g[1][1]), 
        j.yval_plus = DygraphLayout.parseFloat_(g[1][2])), e.push(j);
    }
    return e;
    ...
    for (c = n; c >= 1; c--) if (this.visibility()[c - 1]) {
        ...
        this.attr_("stackedGraph") && Dygraph.stackPoints_(x, h, w, this.attr_("stackedGraphNaNFill")), 
        i[v] = w, g[c] = x;
    }
    return {
        points: g,
    ...
    this.attrs_.pointSize = .5 * this.attr_("highlightCircleSize");
    ...
    this.lastRow_ = -1, this.canvas_.getContext("2d").clearRect(0, 0, this.canvas_.width, this.canvas_.height), 
    null !== this.attr_("drawCallback") && this.attr_("drawCallback")(this, a), a) for (this.readyFired_ = !0; this.readyFns_.length > 0; ) {
        var c = this.readyFns_.pop();
        c(this);
    }
}, Dygraph.prototype.computeYAxes_ = function() {
    var a, b, c, d, e;
    if (void 0 !== this.axes_ && this.user_attrs_.hasOwnProperty("valueRange") === !1) for (a = [], 
    c = 0; c < this.axes_.length; c++) a.push(this.axes_[c].valueWindow);
    ...
    for (b = 0; b < this.axes_.length; b++) if (0 === b) d = this.optionsViewForAxis_("y" + (b ? "2" : "")), 
    e = d("valueRange"), e && (this.axes_[b].valueRange = e); else {
    ...
    for (var b, c, d, e, f, g = function(a) {
        ...
        if (d = this.attributes_.seriesForAxis(i), b = !0, e = .1, null !== this.attr_("yRangePad") && (b = !1, 
        e = this.attr_("yRangePad") / this.plotter_.area.h), 0 === d.length) j.extremeRange = [ 0, 1 ]; else {
            ...
            if (k) if (b) s = q + e * c, t = p; else {
                ...
                s = q * u, t = p / u;
            } else s = q + e * c, t = p - e * c, b && !this.attr_("avoidMinZero") && (0 > t && p >= 0 && (t = 0), 
            s > 0 && 0 >= q && (s = 0));
            j.extremeRange = [ t, s ];
        }
        ...
        if (m) {
            ...
            j.ticks = y(j.computedValueRange[0], j.computedValueRange[1], this.height_, x, this), 
            f || (f = j);
        }
    }
    ...
    for (var d = [], e = this.attr_("errorBars"), f = this.attr_("customBars"), g = 0; g < a.length; g++) {
        ...
        null !== i ? d.push([ h, i ]) : d.push([ h, e ? [ null, null ] : f ? [ null, null, null ] : i ]);
    }
    return d;
    ...
    if (this.fractions_) {
        ...
        for (e = 0; e < a.length; e++) {
            ...
            } else k[e] = [ p, [ 0, 0, 0 ] ]; else j = n ? l * Math.sqrt(q * (1 - q) / n) : 1, 
            k[e] = [ p, [ o * q, o * j, o * j ] ]; else k[e] = [ p, o * q ];
        }
        ...
        for (e = 0; e < a.length; e++) {
            var x = a[e][1];
            if (g = x[1], k[e] = [ a[e][0], [ g, g - x[0], x[2] - g ] ], null === g || isNaN(g) || (c += x[0], 
            v += g, d += x[2], w += 1), e - b >= 0) {
                var y = a[e - b];
                null === y[1][1] || isNaN(y[1][1]) || (c -= y[1][0], v -= y[1][1], d -= y[1][2], 
                w -= 1);
            }
            w ? k[e] = [ a[e][0], [ 1 * v / w, 1 * (v - c) / w, 1 * (d - v) / w ] ] : k[e] = [ a[e][0], [ null, null, null ] ];
        }
    } else if (this.attr_("errorBars")) for (e = 0; e < a.length; e++) {
        h = 0;
        var z = 0;
        for (i = 0, f = Math.max(0, e - b + 1); e + 1 > f; f++) g = a[f][1][0], null === g || isNaN(g) || (i++, 
        h += a[f][1][0], z += Math.pow(a[f][1][1], 2));
        ...
        if (1 == b) return a;
        for (e = 0; e < a.length; e++) {
            for (h = 0, i = 0, f = Math.max(0, e - b + 1); e + 1 > f; f++) g = a[f][1], null === g || isNaN(g) || (i++, 
            h += a[f][1]);
    ...
    a ? (this.attrs_.xValueParser = Dygraph.dateParser, this.attrs_.axes.x.valueFormatter = Dygraph.dateString_, 
    this.attrs_.axes.x.ticker = Dygraph.dateTicker, this.attrs_.axes.x.axisLabelFormatter = Dygraph.dateAxisFormatter) : (this.attrs_.xValueParser = function(a) {
        return parseFloat(a);
    ...
    return null !== c && null !== b && (e += " on line " + (1 + b) + " ('" + c + "') of CSV."), 
    this.error(e), null;
    ...
    for (var i, j = 0, k = !1, l = this.attr_("labels").length, m = !1, n = h; n < f.length; n++) {
        ...
        if (j = n, 0 !== o.length && "#" != o[0]) {
            ...
            if (!(p.length < 2)) {
                var q = [];
                if (k || (this.detectTypeFromString_(p[0]), i = this.attr_("xValueParser"), k = !0), 
                q[0] = i(p[0], this), this.fractions_) for (c = 1; c < p.length; c++) b = p[c].split("/"), 
                2 != b.length ? (this.error('Expected fractional "num/den" values in CSV data but found a value \'' + p[c] + "' on line " + (1 + n) + " ('" + o + "') which is not of this form."), 
                q[c] = [ 0, 0 ]) : q[c] = [ this.parseFloat_(b[0], n, o), this.parseFloat_(b[1], n, o) ]; else if (this.attr_("errorBars")) for (p.length % 2 != 1 && this.error("Expected alternating (value, stdev.) pairs in CSV data but line " + (1 + n) + " has an odd number of values (" + (p.length - 1) + "): '" + o + "'"), 
                c = 1; c < p.length; c += 2) q[(c + 1) / 2] = [ this.parseFloat_(p[c], n, o), this.parseFloat_(p[c + 1], n, o) ]; else if (this.attr_("customBars")) for (c = 1; c < p.length; c++) {
                    var r = p[c];
                ...
                if (d.length > 0 && q[0] < d[d.length - 1][0] && (m = !0), q.length != l && this.error("Number of columns in line " + n + " (" + q.length + ") does not agree with number of labels (" + l + ") " + o), 
    ...
    return m && (this.warn("CSV is out of order; order it correctly to speed loading."), 
    d.sort(function(a, b) {
        return a[0] - b[0];
    })), d;
}, Dygraph.prototype.parseArray_ = function(a) {
    if (0 === a.length) return this.error("Can't plot empty data set"), null;
    if (0 === a[0].length) return this.error("Data set cannot contain an empty row"), 
    null;
    var b;
    if (null === this.attr_("labels")) {
        for (this.warn("Using default labels. Set labels explicitly via 'labels' in the options parameter"), 
        this.attrs_.labels = [ "X" ], b = 1; b < a[0].length; b++) this.attrs_.labels.push("Y" + b);
        this.attributes_.reparseSeries();
    } else {
        var c = this.attr_("labels");
        if (c.length != a[0].length) return this.error("Mismatch between number of labels (" + c + ") and number of columns in array (" + a[0].length + ")"), 
        null;
    }
    if (Dygraph.isDateLike(a[0][0])) {
        this.attrs_.axes.x.valueFormatter = Dygraph.dateString_, this.attrs_.axes.x.ticker = Dygraph.dateTicker, 
        this.attrs_.axes.x.axisLabelFormatter = Dygraph.dateAxisFormatter;
        var d = Dygraph.clone(a);
        for (b = 0; b < a.length; b++) {
            if (0 === d[b].length) return this.error("Row " + (1 + b) + " of data is empty"), 
            null;
            if (null === d[b][0] || "function" != typeof d[b][0].getTime || isNaN(d[b][0].getTime())) return this.error("x value in row " + (1 + b) + " is not a Date"), 
            null;
            d[b][0] = d[b][0].getTime();
        }
        return d;
    ...
    return this.attrs_.axes.x.valueFormatter = function(a) {
        return a;
    }, this.attrs_.axes.x.ticker = Dygraph.numericLinearTicks, this.attrs_.axes.x.axisLabelFormatter = Dygraph.numberAxisLabelFormatter, 
    a;
    ...
    if ("date" == e || "datetime" == e) this.attrs_.xValueParser = Dygraph.dateParser, 
    this.attrs_.axes.x.valueFormatter = Dygraph.dateString_, this.attrs_.axes.x.ticker = Dygraph.dateTicker, 
    this.attrs_.axes.x.axisLabelFormatter = Dygraph.dateAxisFormatter; else {
        if ("number" != e) return this.error("only 'date', 'datetime' and 'number' types are supported for column 1 of DataTable input (Got '" + e + "')"), 
        null;
    ...
    for (f = 0; d > f; f++) {
        ...
        if ("undefined" != typeof a.getValue(f, 0) && null !== a.getValue(f, 0)) {
            if ("date" == e || "datetime" == e ? q.push(a.getValue(f, 0).getTime()) : q.push(a.getValue(f, 0)), 
            this.attr_("errorBars")) for (g = 0; c - 1 > g; g++) q.push([ a.getValue(f, 1 + 2 * g), a.getValue(f, 2 + 2 * g) ]); else {
    ...
    o && (this.warn("DataTable is out of order; order it correctly to speed loading."), 
    n.sort(function(a, b) {
        return a[0] - b[0];
    })), this.rawData_ = n, p.length > 0 && this.setAnnotations(p, !0), this.attributes_.reparseSeries();
}, Dygraph.prototype.start_ = function() {
    var a = this.file_;
    if ("function" == typeof a && (a = a()), Dygraph.isArrayLike(a)) this.rawData_ = this.parseArray_(a), 
    this.predraw_(); else if ("object" == typeof a && "function" == typeof a.getColumnRange) this.parseDataTable_(a), 
    this.predraw_(); else if ("string" == typeof a) {
    ...
    "rollPeriod" in d && (this.rollPeriod_ = d.rollPeriod), "dateWindow" in d && (this.dateWindow_ = d.dateWindow, 
    "isZoomedIgnoreProgrammaticZoom" in d || (this.zoomed_x_ = null !== d.dateWindow)), 
    "valueRange" in d && !("isZoomedIgnoreProgrammaticZoom" in d) && (this.zoomed_y_ = null !== d.valueRange);
    var e = Dygraph.isPixelChangingOptionList(this.attr_("labels"), d);
    Dygraph.updateDeep(this.user_attrs_, d), this.attributes_.reparseSeries(), c ? (this.file_ = c, 
    b || this.start_()) : b || (e ? this.predraw_() : this.renderGraph_(!1));
    ...
    var d = function(a, c, d) {
        ...
        "undefined" != typeof a[c] && (Dygraph.warn("Option " + c + " is deprecated. Use the " + f + " option for the " + e + " axis instead. (e.g. { axes : { " + e + " : { " + f + " : ... } } } (see http://dygraphs.com/per-axis.html for more information."), 
    ...
    if (!this.resize_lock) {
        this.resize_lock = !0, null === a != (null === b) && (this.warn("Dygraph.resize() should be called with zero parameters or two non-NULL parameters. Pretending it was zero."), 
        a = b = null);
    ...
    0 > a || a >= c.length ? this.warn("invalid series number in setVisibility: " + a) : (c[a] = b, 
    this.predraw_());
    ...
    return Dygraph.addAnnotationRule(), this.annotations_ = a, this.layout_ ? (this.layout_.setAnnotations(this.annotations_), 
    void (b || this.predraw_())) : void this.warn("Tried to setAnnotations before dygraph was ready. Try setting them in a ready() block. See dygraphs.com/tests/annotation.html");
...
}, Dygraph.DEBUG = 1, Dygraph.INFO = 2, Dygraph.WARNING = 3, Dygraph.ERROR = 3, 
Dygraph.LOG_STACK_TRACES = !1, Dygraph.DOTTED_LINE = [ 2, 2 ], Dygraph.DASHED_LINE = [ 7, 3 ], 
Dygraph.DOT_DASH_LINE = [ 7, 2, 2, 2 ], Dygraph.log = function(a, b) {
    var c;
    ...
    if (a.offsetParent) for (var c = a; ;) {
        var d = "0";
        if (window.getComputedStyle && (d = window.getComputedStyle(c, null).borderLeft || "0"), 
        b += parseInt(d, 10), b += c.offsetLeft, !c.offsetParent) break;
        c = c.offsetParent;
    } else a.x && (b += a.x);
    ...
    if (a.offsetParent) for (var c = a; ;) {
        var d = "0";
        if (window.getComputedStyle && (d = window.getComputedStyle(c, null).borderTop || "0"), 
        b += parseInt(d, 10), b += c.offsetTop, !c.offsetParent) break;
        c = c.offsetParent;
    } else a.y && (b += a.y);
    ...
    return (!c || isNaN(c)) && Dygraph.error("Couldn't parse " + a + " as a date"), 
    c;
    ...
    for (var b = [], c = 0; c < a.length; c++) Dygraph.isArrayLike(a[c]) ? b.push(Dygraph.clone(a[c])) : b.push(a[c]);
    return b;
    ...
    CIRCLE: function(a, b, c, d, e, f, g) {
        c.beginPath(), c.strokeStyle = f, c.fillStyle = "white", c.arc(d, e, g, 0, 2 * Math.PI, !1), 
        c.fill(), c.stroke();
    },
    STAR: Dygraph.shapeFunction_(5, 0, 4 * Math.PI / 5),
    PLUS: function(a, b, c, d, e, f, g) {
        c.strokeStyle = f, c.beginPath(), c.moveTo(d + g, e), c.lineTo(d - g, e), c.closePath(), 
        c.stroke(), c.beginPath(), c.moveTo(d, e + g), c.lineTo(d, e - g), c.closePath(), 
        c.stroke();
    },
    EX: function(a, b, c, d, e, f, g) {
        c.strokeStyle = f, c.beginPath(), c.moveTo(d + g, e + g), c.lineTo(d - g, e - g), 
        c.closePath(), c.stroke(), c.beginPath(), c.moveTo(d + g, e - g), c.lineTo(d - g, e + g), 
    ...
    for (var a = document.getElementsByTagName("iframe"), b = 0; b < a.length; b++) {
        ...
        h.style.position = "absolute", h.style.left = d + "px", h.style.top = e + "px", 
        h.style.width = f + "px", h.style.height = g + "px", h.style.zIndex = 999, document.body.appendChild(h), 
        this.tarps.push(h);
    }
    ...
    c.isPanning = !0;
    var f = b.xAxisRange();
    if (c.dateRange = f[1] - f[0], c.initialLeftmostDate = f[0], c.xUnitsPerPixel = c.dateRange / (b.plotter_.area.w - 1), 
    ...
    b.dateWindow_ = [ d, e ], c.is2DPan) for (var f = c.dragEndY - c.dragStartY, g = 0; g < b.axes_.length; g++) {
        ...
        n ? h.valueWindow = [ Math.pow(Dygraph.LOG_SCALE, m), Math.pow(Dygraph.LOG_SCALE, l) ] : h.valueWindow = [ m, l ];
    }
    b.drawGraph_(!1);
    ...
    c.isPanning = !1, c.is2DPan = !1, c.initialLeftmostDate = null, c.dateRange = null, 
    c.valueRange = null, c.boundedDates = null, c.boundedValues = null, c.axes = null;
...
Dygraph.SIX_HOURLY = 12, Dygraph.DAILY = 13, Dygraph.WEEKLY = 14, Dygraph.MONTHLY = 15, 
Dygraph.QUARTERLY = 16, Dygraph.BIANNUAL = 17, Dygraph.ANNUAL = 18, Dygraph.DECADAL = 19, 
Dygraph.CENTENNIAL = 20, Dygraph.NUM_GRANULARITIES = 21, Dygraph.SHORT_SPACINGS = [], 
Dygraph.SHORT_SPACINGS[Dygraph.SECONDLY] = 1e3, Dygraph.SHORT_SPACINGS[Dygraph.TWO_SECONDLY] = 2e3, 
Dygraph.SHORT_SPACINGS[Dygraph.FIVE_SECONDLY] = 5e3, Dygraph.SHORT_SPACINGS[Dygraph.TEN_SECONDLY] = 1e4, 
Dygraph.SHORT_SPACINGS[Dygraph.THIRTY_SECONDLY] = 3e4, Dygraph.SHORT_SPACINGS[Dygraph.MINUTELY] = 6e4, 
Dygraph.SHORT_SPACINGS[Dygraph.TWO_MINUTELY] = 12e4, Dygraph.SHORT_SPACINGS[Dygraph.FIVE_MINUTELY] = 3e5, 
Dygraph.SHORT_SPACINGS[Dygraph.TEN_MINUTELY] = 6e5, Dygraph.SHORT_SPACINGS[Dygraph.THIRTY_MINUTELY] = 18e5, 
Dygraph.SHORT_SPACINGS[Dygraph.HOURLY] = 36e5, Dygraph.SHORT_SPACINGS[Dygraph.TWO_HOURLY] = 72e5, 
Dygraph.SHORT_SPACINGS[Dygraph.SIX_HOURLY] = 216e5, Dygraph.SHORT_SPACINGS[Dygraph.DAILY] = 864e5, 
Dygraph.SHORT_SPACINGS[Dygraph.WEEKLY] = 6048e5, Dygraph.LONG_TICK_PLACEMENTS = [], 
Dygraph.LONG_TICK_PLACEMENTS[Dygraph.MONTHLY] = {
    ...
    if (c < Dygraph.MONTHLY) {
        ...
        60 >= j ? (l = k.getSeconds(), Dygraph.setDateSameTZ(k, {
            ...
            m: 0
        }), j /= 60, 24 >= j ? (l = k.getHours(), k.setHours(l - l % j)) : (k.setHours(0), 
        j /= 24, 7 == j && k.setDate(k.getDate() - k.getDay())))), a = k.getTime();
        ...
        for (f = a; b >= f; f += i) {
            if (k = new Date(f), n && k.getTimezoneOffset() != m) {
                ...
                f += 60 * o * 1e3, k = new Date(f), m = k.getTimezoneOffset(), new Date(f + i).getTimezoneOffset() != m && (f += i, 
                k = new Date(f), m = k.getTimezoneOffset());
            }
            h.push({
                v: f,
                label: g(k, c, d, e)
...
Dygraph.Plugins = {}, Dygraph.Plugins.Annotations = function() {
    ...
    return a.prototype.toString = function() {
        ...
        if (b.getOption("drawXAxis")) {
            var d;
            d = b.getOption("xAxisHeight") ? b.getOption("xAxisHeight") : b.getOptionForAxis("axisLabelFontSize", "x") + 2 * b.getOption("axisTickSize"), 
        ...
        if (d.getOption("drawXAxis") || d.getOption("drawYAxis")) {
            ...
            if (d.getOption("drawYAxis")) {
                if (q.yticks && q.yticks.length > 0) {
                    ...
                    for (i = 0; i < q.yticks.length; i++) {
                        ...
                        var w = g - v / 2;
                        0 > w && (w = 0), w + v + 3 > m ? e.style.bottom = "0px" : e.style.top = w + "px", 
                        0 === h[0] ? (e.style.left = r.x - d.getOption("yAxisLabelWidth") - d.getOption("axisTickSize") + "px", 
                        e.style.textAlign = "right") : 1 == h[0] && (e.style.left = r.x + r.w + d.getOption("axisTickSize") + "px", 
                        e.style.textAlign = "left"), e.style.width = d.getOption("yAxisLabelWidth") + "px", 
                        k.appendChild(e), this.ylabels_.push(e);
                    }
                ...
                j.beginPath(), j.moveTo(z, c(r.y)), j.lineTo(z, c(r.y + r.h)), j.closePath(), j.stroke(), 
            ...
            if (d.getOption("drawXAxis")) {
                if (q.xticks) for (i = 0; i < q.xticks.length; i++) {
                    ...
                    B + d.getOption("axisLabelWidth") > l && (B = l - d.getOption("xAxisLabelWidth"), 
                    e.style.textAlign = "right"), 0 > B && (B = 0, e.style.textAlign = "left"), e.style.left = B + "px", 
                    e.style.width = d.getOption("xAxisLabelWidth") + "px", k.appendChild(e), this.xlabels_.push(e);
                }
    ...
    return a.prototype.select = function(a) {
        ...
        if ("undefined" == typeof b) {
            ...
            for (g = a.getOption("labelsSeparateLines"), f = "", h = 1; h < k.length; h++) {
                ...
                l.visible && ("" !== f && (f += g ? "<br/>" : " "), j = a.getOption("strokePattern", k[h]), 
                i = c(j, l.color, e), f += "<span style='font-weight: bold; color: " + l.color + ";'>" + i + " " + k[h] + "</span>");
            }
            return f;
        }
    ...
    return a.prototype.toString = function() {
        ...
        this.initInteraction_(), this.getOption_("animatedZooms") && (this.dygraph_.warn("Animated zooms and range selector are not compatible; disabling animatedZooms."), 
        ...
        function a(a, b) {
            a.style.top = b.y + "px", a.style.left = b.x + "px", a.width = b.w, a.height = b.h, 
            a.style.width = a.width + "px", a.style.height = a.height + "px";
        }
        var b = this.dygraph_.layout_.getPlotArea(), c = 0;
        this.getOption_("drawXAxis") && (c = this.getOption_("xAxisHeight") || this.getOption_("axisLabelFontSize") + 2 * this.getOption_("axisTickSize")), 
        this.canvasRect_ = {
            x: b.x,
            y: b.y + b.h + c + 4,
            w: b.w,
        ...
        if (a || b) {
            ...
            for (var p = 0; p < d.data.length; p++) {
                ...
                isFinite(r) && isFinite(s) ? (null === n ? f.lineTo(r, m) : c && f.lineTo(r, o), 
        ...
        for (d = 0; d < i.length; d++) if (i[d].length > 1 && null !== i[d][1]) {
            if (c = "number" != typeof i[d][1]) for (a = [], b = [], f = 0; f < i[d][1].length; f++) a.push(0), 
            b.push(0);
            break;
        }
        for (d = 0; d < i.length; d++) {
            ...
            for (e = 1; e < l.length; e++) if (this.dygraph_.visibility()[e - 1]) {
                var m;
                if (c) for (f = 0; f < a.length; f++) m = l[e][f], null === m || isNaN(m) || (a[f] += m, 
                b[f]++); else {
        ...
        if (k = this.dygraph_.rollingAverage(k, this.dygraph_.rollPeriod_), "number" != typeof k[0][1]) for (d = 0; d < k.length; d++) h = k[d][1], 
        k[d][1] = h[0];
        var n = Number.MAX_VALUE, o = -Number.MAX_VALUE;
        for (d = 0; d < k.length; d++) h = k[d][1], null !== h && isFinite(h) && (!j || h > 0) && (n = Math.min(n, h), 
        o = Math.max(o, h));
        var p = .25;
